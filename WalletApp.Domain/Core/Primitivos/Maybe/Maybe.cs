...
namespace WalletApp.Domain.Core.Primitivos.Maybe
{
    public sealed class Maybe<T> : IEquatable<Maybe<T>>
    {
        ...
        public T Value => HasValue ? _value : throw new InvalidOperationException("El valor de un Maybe sin valor no puede ser accedido.");

        public static Maybe<T> None => new Maybe<T>(default);

        public static Error NotFound => new Error("SR", "No se encontraron registros.");
